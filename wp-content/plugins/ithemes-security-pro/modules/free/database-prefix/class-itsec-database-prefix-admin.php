...
class ITSEC_Database_Prefix_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Change Database Prefix', 'it-l10n-ithemes-security-pro' ),
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings !== true ) {
			...
			$status       = array(
				'text' => sprintf( '%s wp_.', __( 'Your database table prefix is not using', 'it-l10n-ithemes-security-pro' ) ),
			...
			$status_array = 'medium';
			$status       = array(
				'text' => sprintf( '%s wp_.', __( 'Your database table prefix should not be', 'it-l10n-ithemes-security-pro' ) ),
				'link' => '#itsec_change_table_prefix', 'advanced' => true,
			);
	...
	public function metabox_advanced_settings() {
		...
		$content = '<p>' . __( 'By default, WordPress assigns the prefix "wp" to all tables in the database where your content, users, and objects exist. For potential attackers, this means it is easier to write scripts that can target WordPress databases as all the important table names for 95% of sites are already known. Changing the "wp" prefix makes it more difficult for tools that are trying to take advantage of vulnerabilities in other places to affect the database of your site.', 'it-l10n-ithemes-security-pro' ) . '<strong>' . __( 'Before using this tool, we strongly recommend running a backup of your database.', 'it-l10n-ithemes-security-pro' ) . '</strong></p>';
		...
		if ( isset( $itsec_globals['settings']['write_files'] ) && $itsec_globals['settings']['write_files'] === true ) {
			...
			if ( $this->settings === true ) { //Show the correct info
				...
				?>
				<p><strong><?php _e( 'Your database is using the default table prefix', 'it-l10n-ithemes-security-pro' ); ?>
						<em>wp_</em>. <?php _e( 'You should change this.', 'it-l10n-ithemes-security-pro' ); ?></strong></p>
			<?php
				...
				?>
				<p><?php _e( 'Your current database table prefix is', 'it-l10n-ithemes-security-pro' ); ?>
			...
			<form method="post" action="?page=toplevel_page_itsec_advanced&settings-updated=true" class="itsec-form">
				...
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row" class="settinglabel">
							<label
								for="itsec_change_table_prefix"><?php _e( 'Change Table Prefix', 'it-l10n-ithemes-security-pro' ); ?></label>
						</th>
						<td class="settingfield">
							...
							<p class="description"><?php _e( 'Check this box to generate a new database table prefix.', 'it-l10n-ithemes-security-pro' ); ?></p>
						</td>
					</tr>
					</tbody>
				</table>
				...
				<p class="submit">
					<input type="submit" class="button-primary"
					       value="<?php _e( 'Change Database Prefix', 'it-l10n-ithemes-security-pro' ); ?>"/>
				</p>
			</form>
	...
	public function process_database_prefix() {
		...
		$type    = 'updated';
		$message = __( 'Settings Updated', 'it-l10n-ithemes-security-pro' );
		...
		foreach ( $tables as $table ) {
			...
			if ( $wpdb->query( 'RENAME TABLE `' . $wpdb->base_prefix . $table . '` TO `' . $new_prefix . $table . '`;' ) === false ) {
				...
				$type    = 'error';
				$message = sprintf( '%s %s%s. %s', __( 'Error: Could not rename table', 'it-l10n-ithemes-security-pro' ), $wpdb->base_prefix, $table, __( 'You may have to rename the table manually.', 'it-l10n-ithemes-security-pro' ) );
		...
		if ( $upOpts === false ) { //set an error
			...
			$type    = 'error';
			$message = __( 'Could not update prefix references in options table.', 'it-l10n-ithemes-security-pro' );;
		...
		foreach ( $rows as $row ) {
			...
			if ( substr( $row->meta_key, 0, strlen( $wpdb->base_prefix ) ) == $wpdb->base_prefix ) {
				...
				if ( $result == false ) {
					...
					$type    = 'error';
					$message = __( 'Could not update prefix references in usermeta table.', 'it-l10n-ithemes-security-pro' );
