...
class ITSEC_File_Change {
	...
	function get_email_report( $email_details ) {
		...
		$report .= '<h4>' . __( 'Files Added', 'it-l10n-ithemes-security-pro' ) . '</h4>';
		$report .= '<table border="1" style="width: 100%; text-align: center;">' . PHP_EOL;
		$report .= '<tr>' . PHP_EOL;
		$report .= '<th>' . __( 'File', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		$report .= '<th>' . __( 'Modified', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		$report .= '<th>' . __( 'File Hash', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		$report .= '</tr>' . PHP_EOL;
		...
		if ( isset( $added ) && is_array( $added ) && sizeof( $added > 0 ) ) {
			...
			$report .= '<tr>' . PHP_EOL;
			$report .= '<td colspan="3">' . __( 'No files were added.', 'it-l10n-ithemes-security-pro' ) . '</td>' . PHP_EOL;
			$report .= '</tr>' . PHP_EOL;
		...
		$report .= '<h4>' . __( 'Files Deleted', 'it-l10n-ithemes-security-pro' ) . '</h4>';
		$report .= '<table border="1" style="width: 100%; text-align: center;">' . PHP_EOL;
		$report .= '<tr>' . PHP_EOL;
		$report .= '<th>' . __( 'File', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		$report .= '<th>' . __( 'Modified', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		...
		if ( isset( $removed ) && is_array( $removed ) && sizeof( $removed > 0 ) ) {
			...
			$report .= '<tr>' . PHP_EOL;
			$report .= '<td colspan="3">' . __( 'No files were removed.', 'it-l10n-ithemes-security-pro' ) . '</td>' . PHP_EOL;
			$report .= '</tr>' . PHP_EOL;
		...
		$report .= '<h4>' . __( 'Files Modified', 'it-l10n-ithemes-security-pro' ) . '</h4>';
		$report .= '<table border="1" style="width: 100%; text-align: center;">' . PHP_EOL;
		$report .= '<tr>' . PHP_EOL;
		$report .= '<th>' . __( 'File', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		$report .= '<th>' . __( 'Modified', 'it-l10n-ithemes-security-pro' ) . '</th>' . PHP_EOL;
		...
		if ( isset( $changed ) && is_array( $changed ) && sizeof( $changed > 0 ) ) {
			...
			$report .= '<tr>' . PHP_EOL;
			$report .= '<td colspan="3">' . __( 'No files were changed.', 'it-l10n-ithemes-security-pro' ) . '</td>' . PHP_EOL;
			$report .= '</tr>' . PHP_EOL;
		...
		if ( ! isset( $itsec_globals['settings']['digest_email'] ) || $itsec_globals['settings']['digest_email'] === false ) {
			...
			if ( $changed > 0 ) {
				...
				$message = sprintf(
					...
					__( 'File changes detected', 'it-l10n-ithemes-security-pro' ),
					$itsec_globals['plugin_name'],
					__( 'detected', 'it-l10n-ithemes-security-pro' ),
					$changed,
					__( 'file changes on your system', 'it-l10n-ithemes-security-pro' )
				);
