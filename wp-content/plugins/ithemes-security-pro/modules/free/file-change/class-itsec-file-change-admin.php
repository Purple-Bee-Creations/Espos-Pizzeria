...
class ITSEC_File_Change_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( 'File Change Detection', 'it-l10n-ithemes-security-pro' );
		...
		if ( isset( get_current_screen()->id ) && ( strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) !== false || strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_logs' ) !== false ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'text'                 => __( 'Warning: Your server has less than 128MB of RAM dedicated to PHP. If you have many files in your installation or a lot of active plugins activating this feature may result in your site becoming disabled with a memory error. See the plugin homepage for more information.', 'it-l10n-ithemes-security-pro' ),
					'module_path'          => $this->module_path,
					'button_text'          => isset( $this->settings['split'] ) && $this->settings['split'] === true ? __( 'Scan Next File Chunk', 'it-l10n-ithemes-security-pro' ) : __( 'Scan Files Now', 'it-l10n-ithemes-security-pro' ),
					'scanning_button_text' => __( 'Scanning... (this could take a while on a large site)', 'it-l10n-ithemes-security-pro' ),
					'no_changes'           => __( 'No changes were detected.', 'it-l10n-ithemes-security-pro' ),
					'changes'              => __( 'Changes were detected. Please check the log page for details.', 'it-l10n-ithemes-security-pro' ),
					'error'                => __( 'An error occured. Please try again later', 'it-l10n-ithemes-security-pro' ),
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array(
				'text' => __( 'Your site will detect changes to your files.', 'it-l10n-ithemes-security-pro' ),
			...
			$status_array = 'medium';
			$status       = array(
				'text' => __( 'Your website is not looking for changed files. Consider turning on file change detections.', 'it-l10n-ithemes-security-pro' ),
				'link' => '#itsec_file_change_enabled',
			);
			...
			if ( ! function_exists( 'itsec_intrusion_warning' ) ) {
				...
				function itsec_intrusion_warning() {
					...
					printf(
						...
						__( 'has noticed a change to some files in your WordPress site. Please review the logs to make sure your system has not been compromised.', 'it-l10n-ithemes-security-pro' ),
						__( 'View Logs', 'it-l10n-ithemes-security-pro' ),
						__( 'Dismiss Warning', 'it-l10n-ithemes-security-pro' )
	...
	public function email() {
		...
		$content .= '<label for="itsec_file_change_email"> ' . __( 'Email file change notifications', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description">' . __( 'Notifications will be sent to all emails set to receive notifications on the global settings page.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function enabled() {
		...
		$content .= '<label for="itsec_file_change_enabled"> ' . __( 'Enable File Change detection', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function split() {
		...
		echo '<label for="itsec_file_change_split"> ' . __( 'Split file checking into chunks.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function file_change_form( $origin ) {
		...
		if ( isset( $this->settings['enabled'] ) && $this->settings['enabled'] === true ) {
			...
			echo '<p>' . __( "Press the button below to scan your site's files for changes. Note that if changes are found this will take you to the logs page for details.", 'it-l10n-ithemes-security-pro' ) . '</p>';
			echo '<p><input type="submit" id="itsec_one_time_file_check_submit" class="button-primary" value="' . ( isset( $this->settings['split'] ) && $this->settings['split'] === true ? __( 'Scan Next File Chunk', 'it-l10n-ithemes-security-pro' ) : __( 'Scan Files Now', 'it-l10n-ithemes-security-pro' ) ) . '" /></p>';
	...
	public function file_list() {
		...
		$content = '<p class="description">' . __( 'Exclude files or folders by clicking the red minus next to the file or folder name.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function method() {
		...
		echo '<option value="1" ' . selected( $method, '1' ) . '>' . __( 'Exclude Selected', 'it-l10n-ithemes-security-pro' ) . '</option>';
		echo '<option value="0" ' . selected( $method, '0' ) . '>' . __( 'Include Selected', 'it-l10n-ithemes-security-pro' ) . '</option>';
		echo '</select><br />';
		echo '<label for="itsec_file_change_method"> ' . __( 'Include/Exclude Files', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function notify_admin() {
		...
		$content .= '<label for="itsec_file_change_notify_admin"> ' . __( 'Display file change admin warning', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function types() {
		...
		$content .= '<label for="itsec_file_change_types"> ' . __( 'File types listed here will not be checked for changes. While it is possible to change files such as images it is quite rare and nearly all known WordPress attacks exploit php, js and other text files.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function initialize_admin() {
		...
		add_settings_section(
			'file_change-enabled',
			__( 'File Change Detection', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_section(
			...
			__( 'File Change Detection Settings', 'it-l10n-ithemes-security-pro' ),
			'__return_empty_string',
			'security_page_toplevel_page_itsec_settings'
		);
		...
		add_settings_field(
			...
			__( 'Split File Scanning', 'it-l10n-ithemes-security-pro' ),
			array( $this, 'split' ),
			'security_page_toplevel_page_itsec_settings',
			'file_change-settings'
		);
		...
		add_settings_field(
			...
			__( 'Include/Exclude Files and Folders', 'it-l10n-ithemes-security-pro' ),
			array( $this, 'method' ),
			'security_page_toplevel_page_itsec_settings',
			'file_change-settings'
		);
		...
		add_settings_field(
			...
			__( 'Files and Folders List', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Email File Change Notifications', 'it-l10n-ithemes-security-pro' ),
			array( $this, 'email' ),
			'security_page_toplevel_page_itsec_settings',
			'file_change-settings'
		);
		...
		add_settings_field(
			...
			__( 'Display file change admin warning', 'it-l10n-ithemes-security-pro' ),
