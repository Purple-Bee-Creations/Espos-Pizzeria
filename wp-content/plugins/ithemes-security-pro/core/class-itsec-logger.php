...
final class ITSEC_Logger {
	...
	public function add_admin_meta_boxes() {
		...
		if ( isset( $itsec_globals['settings']['log_type'] ) && ( $itsec_globals['settings']['log_type'] === 0 || $itsec_globals['settings']['log_type'] === 2 ) ) {
			...
			add_meta_box(
				'itsec_log_header',
				__( 'Security Log Information', 'it-l10n-ithemes-security-pro' ),
			...
			add_meta_box(
				'itsec_log_all',
				__( 'Security Log Data', 'it-l10n-ithemes-security-pro' ),
	...
	public function metabox_logs_header() {
		...
		printf(
			'<p>%s %s. %s</p>',
			__( 'Below are various logs of information collected by', 'it-l10n-ithemes-security-pro' ),
			$itsec_globals['plugin_name'],
			__( 'This information can help you get a picture of what is happening with your site and the level of success you have achieved in your security efforts.', 'it-l10n-ithemes-security-pro' )
		);
	...
	public function metabox_logs_header_no_logs() {
		...
		printf(
			'<p>%s</p>',
			__( 'To view logs within the plugin you must enable database logging in the plugin settings. File logging is not available for access within the plugin itself.', 'it-l10n-ithemes-security-pro' )
		);
