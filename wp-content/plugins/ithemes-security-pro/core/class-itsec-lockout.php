...
class ITSEC_Lockout {
	...
	function add_admin_meta_boxes() {
		...
		add_meta_box(
			'itsec_lockouts',
			__( 'Active Lockouts', 'it-l10n-ithemes-security-pro' ),
		...
		foreach ( $lockout_pages as $page ) {
			...
			add_meta_box(
				'itsec_self_protect',
				__( "Don't Lock Yourself Out", 'it-l10n-ithemes-security-pro' ),
	...
	public function admin_script() {
		...
		if ( isset( get_current_screen()->id ) && strpos( get_current_screen()->id, 'itsec' ) !== false ) {
			...
			wp_enqueue_script( 'itsec_temp_whitelist', $itsec_globals['plugin_url'] . 'core/js/admin-whitelist.js', array( 'jquery' ), $itsec_globals['plugin_build'], true );
			wp_localize_script( 'itsec_temp_whitelist', 'itsec_temp_whitelist', array(
				'nonce'   => wp_create_nonce( 'itsec_temp_whitelist_nonce' ),
				'success' => __( 'Temporarily Whitelist my IP', 'it-l10n-ithemes-security-pro' ),
			) );
	...
	public function do_lockout( $module, $user = null ) {
		...
		if ( ! $this->is_ip_whitelisted( $host ) && ( $lock_host !== null || $lock_user !== null || $lock_username !== null ) ) {
			...
			$itsec_logger->log_event( __( 'lockout', 'it-l10n-ithemes-security-pro' ), 10, array( __( 'A whitelisted host has triggered a lockout condition but was not locked out.', 'it-l10n-ithemes-security-pro' ) ), sanitize_text_field( $host ) );
	...
	public function get_lockout_description() {
		...
		$description = sprintf(
			...
			__( 'About Lockouts', 'it-l10n-ithemes-security-pro' ),
			__( 'Your lockout settings can be configured in', 'it-l10n-ithemes-security-pro' ),
			__( 'Global Settings', 'it-l10n-ithemes-security-pro' ),
			__( 'Your current settings are configured as follows:', 'it-l10n-ithemes-security-pro' ),
			__( 'Permanently ban', 'it-l10n-ithemes-security-pro' ),
			( $settings['blacklist'] === true ? __( 'yes', 'it-l10n-ithemes-security-pro' ) : __( 'no', 'it-l10n-ithemes-security-pro' ) ),
			__( 'Number of lockouts before permanent ban', 'it-l10n-ithemes-security-pro' ),
			$settings['blacklist_count'],
			__( 'How long lockouts will be remembered for ban', 'it-l10n-ithemes-security-pro' ),
			...
			__( 'Is this computer white-listed', 'it-l10n-ithemes-security-pro' ),
			( $this->is_ip_whitelisted( ITSEC_Lib::get_ip() ) === true ? __( 'yes', 'it-l10n-ithemes-security-pro' ) : __( 'no', 'it-l10n-ithemes-security-pro' ) )
		);
	...
	public function itsec_temp_whitelist_ajax() {
		...
		if ( $add_temp === false ) {
			...
			$response['message2'] = __( 'is whitelisted until', 'it-l10n-ithemes-security-pro' );
		...
		if ( $itsec_files->get_file_lock( 'lockout_' . $host . $user . $username ) ) {
			...
			if ( $good_host !== false || $good_user !== false || $good_username || $good_username !== false ) {
				...
				if ( $good_host !== false && $blacklist_host === false ) { //temp lockout host
					...
					$itsec_logger->log_event( __( 'lockout', 'it-l10n-ithemes-security-pro' ), 10, array(
	...
	public function lockout_metabox() {
		...
		<form method="post" action="" id="itsec_release_lockout_form">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row" class="settinglabel">
						<?php _e( 'Locked out hosts', 'it-l10n-ithemes-security-pro' ); ?>
					</th>
								...
								<?php foreach ( $host_locks as $host ) { ?>
									<li style="list-style: none;"><input type="checkbox"
										...
										<label
											...
											- Expires <em><?php echo $host['lockout_expire'] ?></em></label>
									</li>
								<?php } ?>
							</ul>
				...
				<tr valign="top">
					<th scope="row" class="settinglabel">
						<?php _e( 'Locked out users', 'it-l10n-ithemes-security-pro' ); ?>
					</th>
					<td class="settingfield">
								...
								<?php foreach ( $user_locks as $user ) { ?>
									...
									<li style="list-style: none;"><input type="checkbox"
										...
										<label
											...
											- Expires <em><?php echo $user['lockout_expire']; ?></em></label>
									</li>
								<?php } ?>
							</ul>
				...
				<tr valign="top">
					<th scope="row" class="settinglabel">
						<?php _e( 'Locked out usernames (not real users)', 'it-l10n-ithemes-security-pro' ); ?>
					</th>
					<td class="settingfield">
						...
						<?php } else { //no user is locked out ?>
							<ul>
								<li style="list-style: none;">
									<p><?php _e( 'Currently no usernames are locked out of this website.', 'it-l10n-ithemes-security-pro' ); ?></p>
								</li>
							</ul>
						<?php } ?>
					</td>
			...
			<p class="submit"><input type="submit" class="button-primary"
			                         value="<?php _e( 'Release Lockout', 'it-l10n-ithemes-security-pro' ); ?>"/></p>
		</form>
	<?php
	}
	...
	public function self_protect_metabox() {
		...
		echo '<p>' . __( 'Security is a delicate item. It does not care who you are, if it sees that you are trying to do something strange it will lock you out. This can be troublesome on sites with existing errors, particularly missing assets such as images and others.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<p>' . __( 'Use the button below to temporarily white list your IP from lockouts for 24 hours. It will still notify you of the situation but it will not lock you out of your site allowing you a chance to fix the issue.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<p>' . __( 'Please note that if your IP address changes at any time during the period (such as you switch locations) you could still inadvertently lock yourself out.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		...
		if ( $temp !== false ) {
			...
			echo __( 'Your IP Address', 'it-l10n-ithemes-security-pro' ) . ', <strong>' . $temp['ip'] . '</strong>, ' . __( 'is whitelisted until', 'it-l10n-ithemes-security-pro' ) . ' <strong>' . date( 'Y-m-d H:i:s', $temp['exp'] ) . '</strong>.<br />';
			echo '<a href="#" class="itsec_temp_whitelist_release_ajax button-primary">' . __( 'Remove IP from Whitelist', 'it-l10n-ithemes-security-pro' ) . '</a>';
			echo '</p>';

		} else {

			echo '<p class="itsec_temp_whitelist submit"><a href="#" class="itsec_temp_whitelist_ajax button-primary">' . __( 'Temporarily Whitelist my IP', 'it-l10n-ithemes-security-pro' ) . '</a></p>';
	...
	private function send_lockout_email( $host, $user, $username, $host_expiration, $user_expiration, $reason ) {
		...
		if ( ! isset( $itsec_globals['settings']['digest_email'] ) || $itsec_globals['settings']['digest_email'] === false ) {
			...
			if ( $host !== false ) {
				...
				$host_text = sprintf( '%s, <a href="http://ip-adress.com/ip_tracer/%s"><strong>%s</strong></a>, ', __( 'host', 'it-l10n-ithemes-security-pro' ), sanitize_text_field( $host ), sanitize_text_field( $host ) );
				...
				if ( $host_expiration === false ) {
					...
					$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the host lockout you can remove the host from the', 'it-l10n-ithemes-security-pro' ), wp_login_url( get_admin_url( '', 'admin.php?page=toplevel_page_itsec_settings' ) ), __( 'host list', 'it-l10n-ithemes-security-pro' ) );

				} else {

					$host_expiration_text .= sprintf( '<strong>%s %s</strong>', __( 'until', 'it-l10n-ithemes-security-pro' ), sanitize_text_field( $host_expiration ) );
					$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockout please visit', 'it-l10n-ithemes-security-pro' ), wp_login_url( get_admin_url( '', 'admin.php?page=itsec' ) ), __( 'the admin area', 'it-l10n-ithemes-security-pro' ) );
			...
			if ( $user_object !== false || $username !== false ) {
				...
				if ( $host_text === '' ) {
					...
					$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockout please visit', 'it-l10n-ithemes-security-pro' ), wp_login_url( get_admin_url( '', 'admin.php?page=itsec' ) ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );
					...
					if ( $host_expiration === false ) {
						...
						$release_text .= sprintf( '%s <a href="%s">%s</a>.', __( 'To release the user lockout please visit', 'it-l10n-ithemes-security-pro' ), wp_login_url( get_admin_url( '', 'admin.php?page=itsec' ) ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );

					} else {

						$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockouts please visit', 'it-l10n-ithemes-security-pro' ), wp_login_url( get_admin_url( '', 'admin.php?page=itsec' ) ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );
