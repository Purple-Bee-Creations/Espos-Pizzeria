 ...
 */ 
	...
	function optionsframework_add_admin() {
		...
		if(function_exists('add_object_page')) {
			$icon = THEME_URI . '/framework/admin/images/cog-icon.png'; // Icon for theme admin menu
	...
	function optionsframework_options_page(){
	    global $options, $advance_options;
	...
	function atp_admin_head() {
		...
		jQuery(document).ready(function(){ 
			...
			jQuery('#sys_add_social_item').click(function() { 
				...
				'<tr>' +
				'<td align="center" width="70"><a href="#" class="sys_social_item_delete button button-primary"><span>Delete</span></a></td>' +
				'<td width="50"><input type="text"  class="sys_social_title" /></td>' +
					...
					echo "<option style=".'background:url('.THEME_URI.'/images/sociables/'.$values.');'." value=".$values.">$values</option>"; } ?></select></td>' +
				...
				case 'custom_socialbook_mark':
						...
						if (get_option('atp_social_bookmark') != '') {
							...
							for($i=0;$i<count($sys_social_items);$i++) {
								...
								$output .= '<tr>';
								$output .= '<td>';
								$output .= '<a href="#" class="sys_social_item_delete button button-primary"><span>Delete</span></a>';
								$output .= '</td>';
								$output .= '<td>';
				...
				case 'customsidebar':
						...
						if($custom_sidebar_arr !=""){
							foreach($custom_sidebar_arr as $custom_sidebar_code) {
								$output.='<tr><td><input type="text" name="'.$value['id'].'[]" value="'. $custom_sidebar_code.'"  size="30" style="width:97%" /></td><td><a class="button button-primary red-button" href="javascript:void(0);return false;" onClick="jQuery(this).parent().parent().remove();"><span>Delete</span></a></td></tr>';
							}
						}				
						$output.='</tbody></table><button type="button" class="button button-primary button-large" name="add_custom_widget" value="Add Sidebar" onClick="addWidgetRow()"><span>Add Sidebar</span></button></div>';
						?>
						<script type="text/javascript" language="javascript">
							function addWidgetRow(){
								jQuery('#custom_widget_table').append('<tr><td><input type="text" name="<?php echo $value['id'];?>[]" value="" size="30" style="width:97%" /></td><td><a class="button button-primary red-button" href="javascript:void(0);return false;" onClick="jQuery(this).parent().parent().remove();"><span>Delete</span></a></td></tr>');
							}
						</script>
						<?php
	...
	if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
		function optionsframework_medialibrary_uploader($_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
			...
			*/
			...
			$output .= '<input id="upload_' . $id . '" class="upload_button button-primary blue-button button-large" type="button" value="' . __( 'Upload', 'optionsframework' ) . '" rel="' . $int . '" />' . "\n";
			...
			if ( $value != '' ) { 
				$remove = '<a href="javascript:(void);" class="custom_clear_image_button button button-primary red-button">X</a>';
